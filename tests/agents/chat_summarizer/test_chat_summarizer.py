...
async def test_first_turn_no_prior_messages(summarizer, mock_llm):
    ...
    assert result.last_messages == [HumanMessage(content="Hello")]
...
async def test_multi_turn_compresses_prior_history(mock_llm):
    ...
    assert result.last_messages == [HumanMessage(content="Third question")]
...
async def test_nothing_new_to_summarize(mock_llm):
    ...
    assert result.last_messages == [HumanMessage(content="Second question")]
...
async def test_tool_call_messages_filtered_out(mock_llm):
    ...
    assert mock_llm.call_count == 1
    assert tool_ai_msg not in result.last_messages
    assert result.last_messages == [HumanMessage(content="Thanks")]
    assert result.span == 2
